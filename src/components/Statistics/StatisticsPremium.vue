        ...
        <table class="full-width">
          ...
          <tbody class="items items-border" v-if="transactions.length > 0">
            <tr v-for="(item, key) in transactions" :key="key">
              <td data-label="TYPE"><div class="">{{ item.type }}</div></td>
              <td data-label="PLAN"><div class="">{{ item.plan }}</div></td>
              <td data-label="AMOUNT"><div class="amount amount-left"><cryptoicon :symbol="item.curr" size="16" /> {{ item.amount.toFixed(8) }}</div></td>
              <td data-label="PRICE"><div class="amount amount-left">${{ item.price.toFixed(2) }}</div></td>
              <td data-label="COUNT"><div class="">x{{ item.count }}</div></td>
  ...
  export default {
    ...
    methods: {
      load: function (page) {
        ...
        this.axios.get(`/api/premium/history/${page}?t=${new Date().getTime()}`)
          .then(response => {
