  ...
  <div>
    ...
    <div v-else class="transactions withdrawal">
      <div class="withdrawal-title">
        <div class="transaction-title">{{ $t('earning.offerWalls.payout') }}</div>
      </div>
      <div class="transaction-table">
        ...
        <table class="full-width campaigns-table">
          ...
          <thead>
          <tr>
            <td>{{ $t('earning.offerWalls.curr') }}</td>
            <td>{{ $t('other.amount') }}</td>
            <td>{{ $t('other.address') }}</td>
            <td>TxID</td>
            <td>{{ $t('earning.offerWalls.status') }}</td>
            <td>{{ $t('other.time') }}</td>
            <td></td>
          </tr>
          </thead>
          ...
          <tr v-for="(item, key) in loadedData" :key="key">
            ...
            <td data-label="TxID">
              <div class="hoverable"><a :href="getTxUrl(item.curr, item.txid)">{{ item.txid }}</a></div>
            </td>
            ...
            <td>
              <div class="edit-buttons">
                <router-link @click.native="copyToChat(item.txid, item.curr)" to="#">
...
export default {
  ...
  methods: {
    load: function (page = 1) {
      ...
      this.axios.get(`/api/payout/history/${page}`)
        .then(response => {
