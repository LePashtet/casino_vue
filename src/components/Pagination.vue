  ...
  <div>
    ...
    <ul v-if="totalPage > 1 && totalPage < 8" class="pagination">
      <li v-if="withNextPrev" :class="disablePrev">
        <a @click="!disablePrev ? btnPrev() : ''" class="button-style">
          <font-awesome-icon icon="chevron-left" />
        </a>
      </li>
      <li v-for="(n, key) in totalPage" :class="n === selected ? 'active ' + activeBGColor  : ''" :key="key">
      ...
      <li v-if="withNextPrev" :class="disableNext">
        <a @click="!disableNext ? btnNext() : ''" class="button-style">
    ...
    <ul v-else-if="totalPage >= 8" class="pagination">
      ...
      <li v-for="(n, key) in pages" v-if="n.show" :class="n.content === selected ? 'active ' + activeBGColor : '' + n.disable"  :key="key">
...
<script>
  export default {
    ...
    methods : {
      ...
      {
        this.selected = n;
        this.$emit('btn-click', n, this.myData);
      ...
      {
        this.selected++;
        this.$emit('btn-click', this.selected, this.myData);
